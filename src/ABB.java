    ...
    //o mesmo valor). Analise a complexidade.
        ...
        public int contarMaioresQueRaiz() {
            if (this.raiz == null) {
                return 0;
            }
            // Dica de ouro: pense bem em qual NÓ você vai passar para o auxiliar
            // para evitar buscar em lugares inúteis da árvore!
            return contarMaioresAUX(this.raiz, (Integer) this.raiz.getItem());
        }

        private int contarMaioresAUX(No<E> raiz, int valorRaiz) {
            if(raiz==null){
                return 0;
            }
            if(raiz.getItem()>valorRaiz){
                return 1 + contarMaioresAUX( raiz.getDireita(),valorRaiz) + contarMaioresAUX( raiz.getEsquerda(),valorRaiz);
            }
            return contarMaioresAUX( raiz.getDireita(),valorRaiz) + contarMaioresAUX( raiz.getEsquerda(),valorRaiz);

        }
        public int contarFolhasPares(){
            if (this.raiz == null) {
                return 0;
            }
            return contarFolhasParesAUX(this.raiz);
        }
        public int contarFolhasParesAUX(No<E> raiz){
            if(raiz==null){
                return 0;
            }
            if(raiz.getEsquerda()==null && raiz.getDireita()==null){
                if((raiz.getItem()%2)== 0){
                    return 1+ contarFolhasParesAUX(raiz.getDireita())+contarFolhasParesAUX(raiz.getEsquerda());
                }else
                    return 0;
            }
